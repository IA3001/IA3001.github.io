...
- 借鉴 SA 构建方式，双关键字基数排序
	- 注意 使用 `tmp` 中介
	- 使用 `cur` 辅助理解
	- 注意 `cnt[256]` 开辟大小 与 **线性（非倍增）** 增量基数排序 的关系
...
- 关于最大串长问题
	- 试想一下长度严格递增，会导致总量 $O(L^2)$ 增长 故最大长度复杂度为 $O(\sqrt{n})$

**可行的状态设计：DP 转移**

- 设 $\text{dp}[i][j]$ 表示最后一个数字以 $i$ 位置结尾，且长度为 $j$ 的最长长度（认为无前导 0）
- 考虑其前驱数字
	- 首先结束位置必须 $\le i-j$ 
	- 其次考虑长度（经过考虑，显然先遍历 $j$ 作为行）
		- 长度 $<j$ 时，必然可以通过 $1 + \max_{x \le i-j, y \lt j} \text{dp}[x][y]$ 转移而来
			- 这是一个 **依赖历史旧行** 的 **本质二维前缀区间最值**，只需要一个辅助数组即可维护，可以滚动
		- 长度 $=j$ 时，需要寻找 $1 + \max_{x \le i-j, y = j, \text{rk}(x) \lt \text{rk}(i)} \text{dp}[x][y]$ 即找到同长度中比它字典序排名小的中的最大的 $\text{dp}$ 值
			- 这是一个 **同行（j）** 且 **第二维偏序查询前缀最值** 的转移方式，显然可以使用树状数组维护，下标表示 $\text{rk}$ ，值表示 $\text{dp}$
- 【注意】`0` 的处理
	- 选取的当前 $i, j$ 对应的最高位如果是 `0`，那么其等价于 `dp[i][j-1]` 的效果，而非真正的按照此 $j$ 长度计算，故选择跳过（直接继承）
	- 【特判之特判 / 无需特判】如果当前的数字只有一个 `0`，即 $j = 1$，此时这里的 `0`，不应该被判定为前导 0，故将其按照正常处理
...
- 证法：特征值+特征多项式+整除性约束
	- 若 $I+kA$ 不可逆则
	- 存在 $A$ 特征值为 $\lambda$ 有
	- $1 + k\lambda = 0 \Rightarrow \lambda = -\frac{1}{k}$ 
	- 而 $A$ 的特征多项式的方程是（显然 $a_i$ 都是整数）
	- $f(x) = a_n x^n + a_{n-1} x^{n-1} + \dots +  a_1 x + a_0 = 0$
	- 代入 $x=-\frac{1}{k}$ 
	- 整理得到
		- $a_n (-1)^n + a_{n-1} (-1)^{n-1} k + a_{n-2} (-1)^{n-2} k^2 + \dots + a_0 k^n = 0$ 
	- 要知道，特征多项式里面， $a_n = 1$ ！！！
	- 于是
	- $(-1)^n = -k(a_{n-1} (-1)^{n-1} + a_{n-2} (-1)^{n-2} k + \dots + a_0 k^{n-1})$ 
	- 注意到有 $k \ge 2$ ，而左边绝对值 $=1$ ，故无解
	- 因此，$I + kA$ 可逆

**正式求解（k 进制）**

- 第 $i$ 轮确定 $k^i$ 同余意义下的答案（直到 $i$ > 值域范围）
	- 首先方程两边同时模 $k$ 得到：
		- $u \equiv v \pmod{k}$
		- 这就是第 $i$ 位的值
	- 设 $v = k w + r$ 
	- 则 $u \equiv r \pmod{k}$ 
	- 这说明，如果只看同余 $k$ ，那么 $r$ 就是一个可行的解
	- 但是，事实远不止如此，因为我们把 $kw$ 给忽略了
	- 现在将 $kw$ 考虑在内，将 $r = kw + r$ 代入方程
	- $u = k(I + kA) w + (I + kA) r$
	- 将 $w$ 化为 $v$ 的样子
	- $\frac{u - (I + kA)r}{k} = (I+kA)w$ 
	- 即我们令
		- $u \leftarrow \frac{u - (I + kA)r}{k}$
		- $v \leftarrow w$
	- 由此解出的 $w$ 即为模 $k$ 意义之外， $v$ 的高层解
- 这个循环求解在干什么？
	- 第 $i$ 轮，确定了 $v$ 的 $k$ 进制第 $i$ 位的值
	- 想想一下，$i=1$ 时的解，精度覆盖不够大
		- 因为 $[-10^9,10^9]$ 范围内，与所求可能解的同余数量巨大
	- 但随着轮数的增加，解的覆盖范围不断扩大
	- 当达到可以覆盖值域同等大小时，必然可以
		- 要么得到唯一的值域内的解（落在值域内）
